...
func ImportTools(data []types.Tool) {
	...
	sql_add_tool := `
		INSERT OR REPLACE INTO nav_table (id, name, catelog, url, logo, desc, req_type, req_method, head, sort, hide)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
		`
	...
	for _, v := range data {
		...
		_, err = stmt.Exec(v.Id, v.Name, v.Catelog, v.Url, v.Logo, v.Desc, v.ReqType, v.ReqMethod, v.Head, v.Sort, v.Hide)
		if err != nil {
			utils.CheckErr(err)
...
func UpdateTool(data types.UpdateToolDto) {
	...
	sql_update_tool := `
		UPDATE nav_table
		SET name = ?, url = ?, logo = ?, catelog = ?, desc = ?, req_type = ?, req_method = ?, head = ?, sort = ?, hide = ?
		WHERE id = ?;
		`
	stmt, err := database.DB.Prepare(sql_update_tool)
	utils.CheckErr(err)
	res, err := stmt.Exec(data.Name, data.Url, data.Logo, data.Catelog, data.Desc, data.ReqType, data.ReqMethod, data.Head, data.Sort, data.Hide, data.Id)
	utils.CheckErr(err)
...
func AddTool(data types.AddToolDto) (int64, error) {
	...
	sql_add_tool := `
		INSERT INTO nav_table (name, url, logo, catelog, desc, req_type, req_method, head, sort, hide)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
		`
	stmt, err := tx.Prepare(sql_add_tool)
	if err != nil {
	...
	res, err := stmt.Exec(data.Name, data.Url, data.Logo, data.Catelog, data.Desc, data.ReqType, data.ReqMethod, data.Head, data.Sort, data.Hide)
	if err != nil {
		return 0, err
	}
...
func GetAllTool() []types.Tool {
	sql_get_all := `
		SELECT id,name,url,logo,catelog,desc,req_type,req_method,head,sort,hide FROM nav_table order by sort;
		`
	...
	for rows.Next() {
		...
		var hide interface{}
		var sort interface{}
		err = rows.Scan(&tool.Id, &tool.Name, &tool.Url, &tool.Logo, &tool.Catelog, &tool.Desc, &tool.ReqType, &tool.ReqMethod, &tool.Head, &sort, &hide)
		if hide == nil {
			tool.Hide = false
		} else {
			if hide.(int64) == 0 {
